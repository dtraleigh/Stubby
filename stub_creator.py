...
class new_media_stub():
    ...
    def ask_and_create(self, env):
        ...
        if not os.path.exists(env.sub_dir):
            os.makedirs(env.sub_dir)

        the_file = open(os.path.join(env.sub_dir, filename), "w")
...
env = environment()
user_input = env.new_stub_or_quit()

while user_input != False:
    if user_input == "c":
        media_stub = new_media_stub()
        media_stub.ask_and_create(env)
    elif user_input == "s":
        env.change_output_folder()
    user_input = env.new_stub_or_quit()
    env.clear_screen()
